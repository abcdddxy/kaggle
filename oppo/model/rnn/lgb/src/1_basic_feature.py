...
def get_pairwise_feature(data, q, t):
    data[q + "_" + t + "_diff_len"] = data[[t + "_len", q + "_len"]].apply(
        lambda x: x[t + "_len"] - x[q + "_len"] if (x[t + "_len"] != -1 and x[q + "_len"] != -1) else -10, axis=1)
    data[q + "_" + t + "_common_seg"] = data.apply(
        lambda x: len(set(x[q].lower().split("|")).intersection(set(x[t].lower().split("|"))) if x[q] != "PAD" and x[t] != "PAD" else []), axis=1)
